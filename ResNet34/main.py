...
# Создание блока ResNet
class ResBlock(nn.Module):
    def __init__(self, nc):
        super().__init__()
        # модуль для создания сверточных слоев в нейронной сети
        # num_ch - количество входных/выходных каналов
        # kernel_size - размер ядра свертки
        # padding - заполнение вокруг матрицы

        # Сверточный слой для входного изображения
        self.conv0 = nn.Conv2d(nc, nc, kernel_size=3, padding=1)
        # Слой нормализации свертки
    ...
    # Применение к выходной переменной out
    def forward(self, x):
        out = self.conv0(x)
        out = self.norm0(out)
        out = self.act(out)
...
class ResNet(nn.Module):
    # Создаем слои и совмещаем блоки
    def __init__(self, in_nc, nc, out_nc):
        super().__init__()
        #  Сверточный слой с ядром 7x7 и шагом 2
        ...
        #  Выходной тензор после применения линейного слоя имеет форму (batch_size, out_nc)
...
def main():
    ...
    # Изменяет скорость обучения в зависимости от номера эпохи
